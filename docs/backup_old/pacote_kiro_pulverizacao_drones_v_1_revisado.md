...
## 19. Lotes de implementação recomendados para geração assistida

A implementação não deve ser gerada como um único bloco monolítico. Para reduzir deriva, facilitar testes e melhorar a qualidade do código, a geração assistida deve ser conduzida em lotes sequenciais, com validação ao final de cada lote.

Princípios:
- cada lote deve produzir incremento utilizável
- cada lote deve preservar compatibilidade com os anteriores
- cada lote deve incluir backend, frontend, validações e persistência quando aplicável
- cada lote deve fechar regras de negócio antes de avançar para o próximo
- não pular o modelo de dados base

### Lote 0 — Fundação e esqueleto do projeto
Objetivo:
Preparar a base técnica e estrutural do sistema.

Escopo:
- criar estrutura do projeto frontend e backend
- configurar PostgreSQL
- configurar migrations
- configurar autenticação
- configurar RBAC por perfil
- criar seed inicial de perfis
- criar layout base da aplicação
- criar navegação principal protegida por perfil
- criar infraestrutura de auditoria básica
- criar componente base de tabela, formulário e modal
- criar componente reutilizável `LocationMapEmbed`
...
- menu inicial por perfil
- fundação pronta para novos módulos

Critério de saída:
- autenticação e autorização funcionando
- migrations executando sem erro
- perfis seedados e utilizáveis
- layout responsivo base disponível

### Lote 1 — Cadastros mestres
Objetivo:
Disponibilizar os dados-base do domínio.
...
- filtros e paginação
- ativação/inativação lógica quando aplicável
- geolocalização básica em cliente, propriedade e talhão conforme modelo
- preview do mapa quando houver coordenadas válidas

Critério de saída:
- CRUD completo dos cadastros mestres
- validações de formulário e backend prontas
- mapas embutidos funcionando nas telas de detalhe

### Lote 2 — Ordem de Serviço
Objetivo:
Iniciar o fluxo operacional formal.
...
- enviar para análise
- aprovar OS
- rejeitar OS com motivo
- cancelar OS com motivo
- listar OS
- detalhar OS
- registrar histórico de status da OS

Requisitos especiais:
- bloqueio de aprovação de OS incompleta
- trilha de auditoria em transições críticas
- código funcional da OS gerado automaticamente

Critério de saída:
- fluxo completo de OS operando do rascunho até aprovação/rejeição/cancelamento
- histórico e auditoria visíveis

### Lote 3 — Planejamento da missão
Objetivo:
Transformar OS aprovada em missão planejada.

Escopo:
- gerar missão a partir da OS aprovada
- herdar dados da OS
- informar parâmetros técnicos
- informar área e volume previstos
- informar restrições e janela operacional
- informar localização operacional
- criar reservas de insumo previstas
- listar e detalhar missões
- registrar histórico de status da missão

Requisitos especiais:
- modelo deve permitir múltiplas missões por OS, mesmo que a UI inicial use uma por padrão
- código funcional da missão gerado automaticamente

Critério de saída:
- missão criada a partir de OS aprovada
- planejamento persistido corretamente
- missões visíveis em lista e detalhe

### Lote 4 — Alocação e agenda
Objetivo:
Preparar a missão para execução.
...
- reservar insumos
- agendar data e hora
- exibir conflitos de agenda básicos
- alterar status para `AGENDADA`

Requisitos especiais:
- bloquear drone indisponível
- bloquear piloto inativo
- bloquear bateria inválida
- alertar conflito simples por faixa de data/hora planejada

Critério de saída:
- missão totalmente alocada e agendada
- conflitos básicos tratados

### Lote 5 — Checklist e liberação
Objetivo:
Garantir prontidão operacional antes da execução.

Escopo:
- gerar checklist da missão com base nos itens padrão
- preencher checklist
- salvar rascunho do checklist
- concluir checklist
...
Critério de saída:
- checklist completo funcionando
- missão liberada apenas quando cumprir regras

### Lote 6 — Execução operacional
Objetivo:
Registrar a operação realizada em campo.

Escopo:
- iniciar missão
- pausar missão com motivo
- retomar missão
- registrar início e fim
- registrar área realizada
- registrar volume realizado
- registrar consumo real de insumos
- registrar observações de execução
- registrar ocorrências
- anexar evidências
- finalizar missão

Requisitos especiais:
- mudança automática do status do drone
- separação obrigatória entre previsto e realizado
- permitir excesso de consumo apenas com justificativa

Critério de saída:
- missão consegue ser executada do início ao fim
- consumo, ocorrências e evidências ficam persistidos

### Lote 7 — Frota e manutenção
Objetivo:
Dar controle operacional mínimo sobre ativos.

Escopo:
- visualizar drones e status
- registrar manutenção
- controlar horas voadas
- controlar baterias e situação
- refletir disponibilidade dos ativos nas alocações

Critério de saída:
- frota e manutenção básicas funcionando
- restrições de alocação respeitando estado do ativo

### Lote 8 — Encerramento técnico e financeiro
Objetivo:
Fechar a missão formalmente.

Escopo:
- encerramento técnico por usuário técnico
- registro de observações técnicas
- criação/edição do financeiro da missão
- custo estimado
- custo realizado
- valor faturado
- status financeiro
...
Requisitos especiais:
- fechamento financeiro depende do encerramento técnico
- encerramento deve registrar usuário e timestamp
- reabertura deve gerar auditoria

Critério de saída:
- missão pode ser encerrada tecnicamente e financeiramente com regras válidas

### Lote 9 — Dashboards e relatórios
Objetivo:
Disponibilizar visão mínima gerencial e operacional.
...
- custo, faturamento e margem por missão
- filtros por período, cliente, propriedade, piloto, drone e status

Critério de saída:
- dashboards e relatórios básicos funcionando com dados reais do sistema

### Lote 10 — Endurecimento para PRD
Objetivo:
Preparar a V1 para implantação mais segura.

Escopo:
- revisar validações backend
- revisar permissões por perfil
- revisar mensagens de erro
- revisar paginação e filtros
- revisar auditoria
- revisar consistência de status
- revisar experiência responsiva para operação em campo
- revisar seeds iniciais e dados de apoio
- melhorar logs e tratamento de exceções

Critério de saída:
- sistema consistente para homologação avançada e preparação de produção

---

## 20. Estratégia de uso do Kiro por lote

Para cada lote, usar o Kiro com a seguinte disciplina:

1. informar claramente o lote alvo
2. anexar este pacote como base mandatória
3. pedir ao Kiro que implemente apenas o lote corrente
4. exigir que ele preserve compatibilidade com o modelo e fluxos já definidos
5. exigir lista explícita de arquivos criados/alterados
6. exigir resumo das decisões tomadas
7. exigir que não implemente escopo de lotes futuros sem instrução

Prompt operacional recomendado por lote:

Implemente apenas o lote [NOME_DO_LOTE] desta especificação. Respeite integralmente o modelo de dados, os estados, as regras de negócio e as decisões já fechadas. Não antecipe funcionalidades de lotes futuros além do estritamente necessário para compatibilidade estrutural. Ao final, informe:
- arquivos criados
- arquivos alterados
- migrations criadas
- endpoints adicionados
- telas/componentes adicionados
- regras de negócio implementadas
- pendências ou limitações

---

## 21. Critérios de revisão após cada lote

Ao final de cada lote, revisar:
- aderência ao MER orientado
- aderência aos status obrigatórios
- aderência às permissões por perfil
- aderência às regras de negócio críticas
- separação correta entre previsto e realizado
- consistência visual mínima
- responsividade mínima
- ausência de invenções fora do escopo
- ausência de simplificações indevidas do domínio

Checklist de revisão:
- o Kiro respeitou o domínio?
- o Kiro criou entidades extras indevidas?
- o Kiro fundiu etapas que deveriam ser separadas?
- o Kiro removeu alguma validação crítica?
- o Kiro mudou nomes ou cardinalidades importantes?
- o Kiro implementou algo fora do lote?

---

## 22. Ordem prática recomendada de geração

Ordem sugerida real de execução:
1. Lote 0
2. Lote 1
3. Lote 2
4. Lote 3
...
Se precisar acelerar prova de valor visual, é aceitável antecipar parte do dashboard simples após o Lote 3, desde que isso não quebre a priorização do fluxo operacional.

---

## 23. Recomendação final de execução

A melhor abordagem é tratar este projeto como geração assistida incremental, com validação contínua. Não pedir ao Kiro “o sistema completo de uma vez”. A qualidade tende a ser muito melhor quando o domínio é implementado em blocos controlados, especialmente em sistemas com estados, aprovações, auditoria e regras operacionais como este.
